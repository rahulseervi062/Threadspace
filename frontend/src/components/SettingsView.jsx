...
export function SettingsView({
  ...
  const themes = [
    { id: "classic", name: "Classic Teal", color: "hsl(172, 78%, 50%)" },
    { id: "cyberpunk", name: "Cyberpunk", color: "hsl(320, 100%, 60%)" },
    { id: "deep-ocean", name: "Deep Ocean", color: "hsl(200, 100%, 50%)" },
    { id: "emerald-forest", name: "Emerald Forest", color: "hsl(145, 80%, 45%)" }
  ];
  ...
  return (
    <div className="content-card">
      ...
      <div style={{ display: "grid", gap: 24 }}>
        <section style={{ padding: "24px", background: "var(--bg-dark)", borderRadius: "14px", border: "1px solid var(--border)" }}>
          ...
          <form className="settings-grid" onSubmit={handleProfileSave}>
            ...
            <label>
              <span>Username</span>
              <div style={{ position: "relative" }}>
                <span style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", color: "var(--accent)", fontWeight: 700 }}>@</span>
        ...
        <section style={{ padding: "24px", background: "var(--bg-dark)", borderRadius: "14px", border: "1px solid var(--border)" }}>
          <h3 style={{ marginBottom: 8, fontSize: "1.1rem", display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <p style={{ color: "var(--text-muted)", fontSize: "0.9rem" }}>Personalize Pulse with a curated color theme.</p>
          <div className="theme-selector-grid">
            {themes.map(t => (
              <div 
                key={t.id} 
                className={`theme-preview-card ${currentTheme === t.id ? "active" : ""}`}
