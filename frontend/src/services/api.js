...
export const api = {
  ...
  // Posts
  getPosts: (page = 1, limit = 10, sort = "new") => fetch(`${API_BASE}/api/posts?page=${page}&limit=${limit}&sort=${sort}`).then(handleResponse),
  ...
  getFollowingPosts: (email) => fetch(`${API_BASE}/api/posts/following?userEmail=${encodeURIComponent(email)}`).then(handleResponse),
  ...
  editComment: (postId, commentId, userEmail, text) => fetch(`${API_BASE}/api/posts/${postId}/comments/${commentId}`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ userEmail, text })
  }).then(handleResponse),
  deleteComment: (postId, commentId, userEmail) => fetch(`${API_BASE}/api/posts/${postId}/comments/${commentId}?userEmail=${encodeURIComponent(userEmail)}`, {
    method: "DELETE"
  }).then(handleResponse),
  reportComment: (postId, commentId, userEmail, reason) => fetch(`${API_BASE}/api/posts/${postId}/comments/${commentId}/report`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  ...
  createGroup: (groupData) => fetch(`${API_BASE}/api/groups`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  ...
  getReports: () => fetch(`${API_BASE}/api/mod/reports`).then(handleResponse),
  ...
  searchGifs: async (query = "trending", limit = 12) => {
    // Use backend proxy to hide Giphy API key
    const params = new URLSearchParams({ q: query, limit: String(limit) });
    const response = await fetch(`${API_BASE}/api/giphy/search?${params.toString()}`);
    if (!response.ok) {
      throw new Error("Failed to search GIFs");
    }
    const data = await response.json();
    if (!data.ok) {
      // Return empty list with message for UI handling
      return { ok: false, gifs: [], message: data.message || "GIPHY not configured" };
    }
    return { ok: true, gifs: data.gifs };
  },
