...
app.get("/api/messages", async (req, res) => {
  ...
  messages.forEach((msg) => {
    ...
    const otherEmailRaw = normalizedFromEmail === userEmail ? msg.toEmail : msg.fromEmail;
    const otherEmailKey = normalizeEmail(otherEmailRaw);

    if (!convMap[otherEmailKey]) {
      const otherUser = users.find((u) => normalizeEmail(u.email) === otherEmailKey);
      convMap[otherEmailKey] = {
        otherEmail: otherEmailKey,
        otherName: otherUser?.name || otherEmailRaw,
        messages: [],
        lastAt: msg.createdAt,
        unread: 0
      };
    }
    convMap[otherEmailKey].messages.push(msg);
    if (normalizedToEmail === userEmail && !msg.read) convMap[otherEmailKey].unread++;
    if (msg.createdAt > convMap[otherEmailKey].lastAt) convMap[otherEmailKey].lastAt = msg.createdAt;
  });
...
app.get("/api/messages/:otherEmail", async (req, res) => {
  ...
  const unreadQuery = {
      fromEmail: { $regex: new RegExp(`^${canonicalOtherEmail}$`, "i") },
      toEmail: { $regex: new RegExp(`^${currentEmail}$`, "i") },
      read: false
  };
